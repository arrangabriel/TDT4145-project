...
def review() -> bool:
    ...
    cursor.execute(
        ...
        VALUES (?,?,?,?,?,?)""",
        [currentUser["email"], roastery, coffeeName, notes, points, datetime.now()])
...
def viewUsers() -> None:
    ...
    cursor.execute(
        ...
        NATURAL JOIN User
        GROUP BY email
        ORDER BY noTastings DESC""")

    for user in cursor.fetchall():
        print(f"{user[2]} {user[3]}: {user[0]} tastings")

def filter():
    countries = input('Which countries would you like to see coffees from?\nProvide a comma separated list: ')
    ...
    excludedProcessingMethod = input('Which processing method do you want excluded (enter to include all): ')
